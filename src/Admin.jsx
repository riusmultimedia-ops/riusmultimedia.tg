...
export default function Admin() {
  ...
  const itemRef = (key) => (el) => {
    if(el){
      ...
      if(pendingScrollKeyRef.current === key){
        ...
        scrollElementIntoView(el, 16, 'center');
      }
    } else {
  ...
  // amenerait le formulaire ou la ligne juste dessous, cache derriere. On utilise la vraie
  // methode du navigateur (elle trouve toute seule le bon bloc qui defile, que ce soit la
  // fenetre entiere ou une zone interieure) et on lui dit juste de s'arreter un peu plus tot,
  // pour laisser la place a l'en-tete.
  const stickyHeaderHeight = () => {
    const h = document.querySelector('[data-rius-admin-header]');
    return h ? h.getBoundingClientRect().height : 0;
  };
  const scrollElementIntoView = (el, extra=16, block='start', instant=false) => {
    el.style.scrollMarginTop = (stickyHeaderHeight() + extra) + 'px';
    el.scrollIntoView({ behavior: instant? 'auto' : 'smooth', block });
  };
  // Le tout premier saut est instantane (on veut voir le formulaire ou la ligne des le clic,
  // sans attendre une animation). On refait ensuite l'appel plusieurs fois de suite (image qui
  // charge, liste qui se retrie...) : chaque correction est quasi invisible si l'element n'a
  // pas bouge, et rattrape silencieusement les cas ou il a bouge entre-temps.
  const settleScroll = (getEl, block, tries=10, delay=120) => {
    if(settleTimerRef.current) clearTimeout(settleTimerRef.current);
    let n = 0;
    const step = () => {
      const el = getEl();
      if(el) scrollElementIntoView(el, 16, block, n===0);
      n++;
  ...
  const goToForm = (name) => {
    settleScroll(() => formRefsMap.current[name], 'start');
  };
  const goToItem = (key) => {
    setFlashKey(key);
    if(flashTimerRef.current) clearTimeout(flashTimerRef.current);
    settleScroll(() => itemRefsMap.current[key], 'center');
    ...
    <div style={{minHeight:'100vh', background:'#eef2ff', fontFamily:'Inter, Arial'}}>
      ...
      <div style={{maxWidth:900, margin:'20px auto', padding:'0 12px'}}>
        <div style={{display:'flex', gap:6, marginBottom:16, overflowX:'auto', flexWrap:'wrap'}}>
          ...
          {canAccess('envois') && <button onClick={()=>{const v=!showEnvois; resetTabs(); setShowEnvois(v); if(v) fetchSubmissions();}} style={{flex:'1 0 100px', padding:'10px', fontSize:11, borderRadius:10, border:'1px solid #86efac', background: showEnvois? '#16a34a':'white', color: showEnvois? 'white':'#16a34a', fontWeight:800}}>📤 Envois reçus ({submissions.filter(s=>s.status!=='traite').length})</button>}
        ...
        {showUsers? (
          ...
          <div style={{background:'white', padding:16, borderRadius:14, borderTop:'4px solid #16a34a'}}>
            ...
            <div ref={formRef('radio')} style={{border:'2px dashed #86efac', padding:12, borderRadius:12, background:'#f0fdf4', marginBottom:12}}>
              ...
              <label style={{display:'flex',alignItems:'center',gap:8,fontSize:11,fontWeight:800,color:'#2563eb',marginBottom:newRadioIsHourly?4:10,cursor:'pointer'}}>
                <input type="checkbox" checked={newRadioIsHourly} onChange={e=>setNewRadioIsHourly(e.target.checked)} />
                C'est un top horaire (se diffuse a une heure precise, chaque jour)
              </label>
              {newRadioIsHourly && (
                <div style={{marginBottom:10}}>
                  <label style={{fontSize:10,fontWeight:800,color:'#2563eb'}}>A QUELLE HEURE DOIT JOUER CE FICHIER ? *</label>
                  <select value={newRadioHourlyHour} onChange={e=>setNewRadioHourlyHour(e.target.value)} style={{width:'100%',padding:8,marginTop:4,borderRadius:8,border:'1px solid #bfdbfe',fontSize:12}}>
