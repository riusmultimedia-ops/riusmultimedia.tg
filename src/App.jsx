...
export default function App(){
  ...
  const [meteo,setMeteo]=useState({temp:'32C',icon:'Soleil'})
  ...
  const globalCss = "@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&display=swap'); @keyframes defile{0%{transform:translateX(100%)}100%{transform:translateX(-100%)}} @keyframes pulse{0%{box-shadow:0 0 0 0 rgba(255,59,59,0.7)}70%{box-shadow:0 0 0 10px rgba(255,59,59,0)}100%{box-shadow:0 0 0 0 rgba(255,59,59,0)}} @keyframes blink{0%,50%{opacity:1}51%,100%{opacity:0}} .dot{width:8px;height:8px;background:#ff3b3b;border-radius:50%;display:inline-block;animation:pulse 2s infinite} .dot-blink{width:7px;height:7px;background:#ff3b3b;border-radius:50%;display:inline-block;animation:blink 1s infinite} .live-text{animation:defile 120s linear infinite;white-space:nowrap;display:inline-block} .live-text2{animation:defile 90s linear infinite;white-space:nowrap;display:inline-block} .yellow-dot{width:8px;height:8px;background:#ffcc00;border-radius:50%;display:inline-block} .nav-white:hover{color:#ffcc00!important} .sep-small{color:rgba(255,255,255,0.5);font-size:9px;margin:0 2px}"
  ...
  useEffect(()=>{ if(typeof window==='undefined') return; localStorage.setItem('rius_lang',lang); },[lang])
  ...
  const articlePrincipal=filteredArticles[0]? getTranslated(filteredArticles[0]):null
  const autres=filteredArticles.slice(1,5).map(getTranslated)
  ...
  return (
    <div style={{margin:0,fontFamily:'Inter,Arial,sans-serif',background:'#162f6b',minHeight:'100vh'}}>
      ...
      <div style={{position:'sticky',top:0,zIndex:1000,width:'100%'}}>
        <div style={{background:'black',color:'white',padding:'0 10px',fontSize:11,display:'flex',gap:8,alignItems:'center',overflow:'hidden',height:26}}>
        ...
        <div style={{background:'white',color:'#0f2040',fontWeight:900,fontSize:11,display:'flex',alignItems:'center',height:26,overflow:'hidden'}}>
          <div style={{background:'#0f2040',color:'#ffcc00',padding:'0 12px',height:'100%',display:'flex',alignItems:'center',gap:6,flexShrink:0}}>ANNONCES</div>
          <div style={{flex:1,overflow:'hidden',display:'flex',alignItems:'center'}}><div className="live-text2" style={{display:'flex',alignItems:'center',fontWeight:700,color:'#333',whiteSpace:'nowrap'}}>{annoncesList.map((txt,i)=>(<span key={i} style={{marginRight:50,whiteSpace:'nowrap',display:'inline-flex',alignItems:'center',gap:8}}><span style={{width:6,height:6,background:'#2e4fb0',borderRadius:'50%',display:'inline-block'}}></span>{txt}</span>))}</div></div>
          <div style={{display:'flex',gap:10,padding:'0 12px',flexShrink:0,borderLeft:'2px solid #eee',fontSize:10,background:'#f8f8f8',height:'100%',alignItems:'center'}}><span>Lome {meteo.temp}</span><span style={{background:'#0f2040',color:'#ffcc00',padding:'3px 10px',borderRadius:10}}>{heureTU}</span></div>
        </div>
          ...
          <div style={{display:'flex',alignItems:'center',overflowX:'auto'}}>
            {CATS.map((item,idx)=>(
              <div key={item} style={{display:'flex',alignItems:'center',flexShrink:0}}>
                <a href="#" className="nav-white" onClick={(e)=>{e.preventDefault();setActif(item)}} style={{color:actif===item?'#ffcc00':'white',textDecoration:'none',padding:'0 7px',fontSize:10,fontWeight:900,whiteSpace:'nowrap',height:32,display:'flex',alignItems:'center',borderBottom:actif===item?'3px solid #ffcc00':'3px solid transparent'}}>{item}</a>
                {idx<CATS.length-1&&<span className="sep-small">|</span>}
              </div>
            ))}
            <span style={{margin:'0 8px',height:22,width:1,background:'rgba(255,255,255,0.6)',display:'inline-block'}}></span>
            <a href="#" onClick={(e)=>{e.preventDefault();setActif('DIRECT')}} style={{color:'#ff3b3b',textDecoration:'none',padding:'0 7px',fontSize:10,fontWeight:900,display:'flex',alignItems:'center',gap:4,height:32,borderBottom:actif==='DIRECT'?'3px solid #ff3b3b':'3px solid transparent'}}><span className="dot-blink"></span>DIRECT</a>
            <span className="sep-small">|</span>
            <a href="#" onClick={(e)=>{e.preventDefault();setActif('PODCAST')}} style={{color:'#a8ff00',textDecoration:'none',padding:'0 7px',fontSize:10,fontWeight:900,height:32,display:'flex',alignItems:'center',borderBottom:actif==='PODCAST'?'3px solid #a8ff00':'3px solid transparent'}}>PODCAST</a>
            <span className="sep-small">|</span>
            <a href="#" onClick={(e)=>{e.preventDefault();setActif('CONTACT')}} style={{color:'#ffcc00',textDecoration:'none',padding:'0 7px',fontSize:10,fontWeight:900,height:32,display:'flex',alignItems:'center',borderBottom:actif==='CONTACT'?'3px solid #ffcc00':'3px solid transparent'}}>CONTACT</a>
          </div>
          ...
          <div style={{maxWidth:1400,margin:'0 auto'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:16}}>
              {unes.length>0 ? unes.map((une)=>(
                ...
                <div style={{gridColumn:'1/-1',textAlign:'center',padding:40,background:'rgba(255,255,255,0.05)',borderRadius:12}}>
                  <div style={{fontSize:14,opacity:0.8}}>Aucune Une active pour le moment</div>
                  <div style={{fontSize:11,opacity:0.5,marginTop:8}}>Verifie dans Supabase que active=true et dans la console F12 le log UNES chargees</div>
                </div>
              )}
            </div>
          </div>
